...
def start(message):
    if user[str(message.from_user.id)][1] == 4 and user[str(message.from_user.id)][0] == "adm":
...
def get_text_messages(message):
    ...
    if user[str(message.from_user.id)][1] == 0:
        bot.send_message(message.from_user.id, "Подтвердите что ваша фамилия:" + message.text)
...
def get_document_messages(message):
    ...
    if user[str(message.from_user.id)][1] == "Присылает таблицу" and user[str(message.from_user.id)][0] == "adm":
        ...
        for Id, value in user_change.items():
            print(Id)
            if datetime.now().timetuple().tm_mon >= int(mounth) and datetime.now().timetuple().tm_mday >= int(Date) and datetime.now().timetuple().tm_hour + 3 >= int(time_rass) and user_change[str(Id)][0][1] != "00:01":
